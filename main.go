...
func main() {
	...
	disordererd := []int{9, 1, 8, 2, 4, 19, 13, 75, 78}
	ordered := quickSort(disordererd)
...
func quickSort(source []int) []int {
	...
	quickSortRecursive(&destination, 0, len(destination)-1)
	return destination
}

func quickSortRecursive(array *[]int, left, right int) {
	if left >= right {
		return
	}
	indexPivot := left + (right-left)/2
